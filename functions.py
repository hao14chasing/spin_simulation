...
"""A function to visualize given rows of a specific column of the spin data chain"""
"""Usage Function"""
def spin_y_visualization_rows(dict_spin, dict_position): 
    ### dict_position is a dictionary with keys as column index and values as a list of row indices
    ### Column is a number, row list is a list of indices
    ### First item in row list is the first row, second item is the final row
    ### dict_position = {1: [1, 2], 2: [1, 2], 3: [1, 2]}
    ### Note: Both column and row index is 1 lager than the actual index (Being discussed)
...
def spin_grid_Sz_visualization(dict_spin):
    ...
    plt.grid(True)
    plt.xticks(np.arange(0, 101, 10))  # Set x-axis ticks at intervals of 10
    plt.yticks(np.arange(0, 101, 10))  # Set y-axis ticks at intervals of 10
